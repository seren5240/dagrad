...
def run_one_experiment(trials, n, s0_ratio, noise_type, error_var):
    ...
    for d in num_nodes:
        ...
        for i in range(trials):
            ...
            try:
                ev_result = golem_ev(n=n, d=d, s0=s0, graph_type="SF", error_var=error_var, noise_type=noise_type)
                shd_results["GOLEM-EV"][d].append(ev_result["shd"] / s0)
                sid_results["GOLEM-EV"][d].append(ev_result["sid"] / s0)

                nv_result = golem_nv(n=n, d=d, s0=s0, graph_type="SF", error_var=error_var, noise_type=noise_type)
...
def make_one_plot(s0_ratio, noise_type, methods, num_nodes, trials, n, error_var, results, metric: str):
    ...
    plt.title(f"{noise_names[noise_type]} Noise, SF{s0_ratio}\n(n={n}, trials={trials}, error_var={error_var})")
    ...
    plt.tight_layout()
    plt.savefig(f"golem_{metric}_SF{format_ratio(s0_ratio)}_noise={noise_type}_n={n}_var={error_var}.png")

    output_filename = f"golem_{metric}_SF{format_ratio(s0_ratio)}_noise={noise_type}_n={n}_var={error_var}.txt"
        ...
        for sem_type in noise_types:
            for s0_ratio in s0_ratios:
                ...
                for _ in range(trials):
                    try:
                        ev_result = golem_ev(n=n, d=d, s0=s0, graph_type="SF", error_var=error_var, noise_type=sem_type)
                        ...
                        nv_result = golem_nv(n=n, d=d, s0=s0, graph_type="SF", error_var=error_var, noise_type=sem_type)
...
def make_subplots(s0_ratios, noise_types, methods, num_nodes, trials, n, error_var, results, metric: str):
    ...
    for i, s0_ratio in enumerate(s0_ratios):
        for j, noise in enumerate(noise_types):
            ...
            ax.set_title(f"{noise_names[noise]} noise, SF{s0_ratio}")
