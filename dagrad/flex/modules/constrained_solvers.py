...
def compute_loss(x, mask, model, weights, biases,mean_std=False):
    # TODO: add param
    """
...
class DCDIAugmentedLagrangian(ConstrainedSolver):
    def __init__(
        self,
        num_iter: int = 20,
        num_steps: Optional[Union[List[int], int]] = None,
        ...
        for i in tqdm(range(self.num_iter)):
            ...
            while self.rho < self.rho_max:
                def augmented_loss(output, target):
                    ...
                    loss = compute_loss(target, masks, model, weights, biases)
